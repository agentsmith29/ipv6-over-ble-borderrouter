...
{
	printk(KERN_INFO "In l2cap_chan_connect");
	...
	printk(KERN_INFO "l2cap_chan_connect 1: %pMR -> %pMR (type %u) psm 0x%2.2x", &chan->src, dst,
			dst_type, __le16_to_cpu(psm));

	hdev = hci_get_route(dst, &chan->src, chan->src_type);
    printk(KERN_INFO "*** l2cap_chan_connect BDAddress of hcidev: %pMR -> %pMR (type %u) psm 0x%2.2x", &hdev->bdaddr, dst,
            dst_type, __le16_to_cpu(psm));
    ...
    printk(KERN_INFO "Set destination address and psm");
    printk(KERN_INFO "l2cap_chan_connect 3: %pMR -> %pMR (type %u) psm 0x%2.2x", &chan->src, dst,
            dst_type, __le16_to_cpu(psm));

	if (bdaddr_type_is_le(dst_type)) {
		/* Convert from L2CAP channel address type to HCI address type
		 */
        printk(KERN_INFO " Convert from L2CAP channel address type to HCI address type");
		...
		if (hci_dev_test_flag(hdev, HCI_ADVERTISING))
			...
			hcon = hci_connect_le_scan(hdev, dst, dst_type,
						   chan->sec_level,
						   HCI_LE_CONN_TIMEOUT);
        printk(KERN_INFO "l2cap_chan_connect 4: %pMR -> %pMR (type %u) psm 0x%2.2x", &chan->src, dst,
                dst_type, __le16_to_cpu(psm));
    ...
    printk(KERN_INFO "Update source addr of the socket");
    printk(KERN_INFO "l2cap_chan_connect 5: %pMR -> %pMR (type %u) psm 0x%2.2x", &chan->src, dst,
            dst_type, __le16_to_cpu(psm));
...
chan_unlock:
	...
	hci_dev_unlock(hdev);
	hci_dev_put(hdev);
    printk(KERN_INFO "l2cap_chan_connect END: %pMR -> %pMR (type %u) psm 0x%2.2x", &chan->src, dst,
            dst_type, __le16_to_cpu(psm));
	return err;
}
