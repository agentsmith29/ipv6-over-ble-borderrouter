...
static struct proc_dir_entry *lowpan_enable_procfs;
static struct proc_dir_entry *lowpan_control_procfs;


struct proc_dir_entry *ble_parent;
...
static int bt_6lowpan_connect(bdaddr_t *addr, u8 dst_type)
{
	...
	printk(KERN_INFO "We are here in connect");
	struct l2cap_chan *chan;
	int err;
	...
	printk(KERN_INFO "chan %p err %d", chan, err);
	if (err < 0)
		l2cap_chan_put(chan);
...
/* FIXME We will add another fix from https://lkml.org/lkml/2018/1/16/1107 */
static int get_l2cap_conn(char *buf, bdaddr_t *addr, u8 *addr_type,
			  struct l2cap_conn **conn)
{
	struct hci_conn *hcon;
	...
	//int hci_dev_id;
	int n;
	u8 lookup_type; // The fix
	n = sscanf(buf, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx %hhu",
		   &addr->b[5], &addr->b[4], &addr->b[3],
		   &addr->b[2], &addr->b[1], &addr->b[0],
		   addr_type);

	printk(KERN_INFO "Selected Address: %pMR.\n", &addr);

	// FIXME Convert from L2CAP channel address type to HCI address type //
	...
	// The LE_PUBLIC address type is ignored because of BDADDR_ANY //
    printk(KERN_INFO "get_l2cap_conn: hci_get_route with addr %pMR", &addr);
	hdev = hci_get_route(addr, BDADDR_ANY, BDADDR_LE_PUBLIC);
	if (!hdev)
		return -ENOENT;

	hci_dev_lock(hdev);
	//hcon = hci_conn_hash_lookup_le(hdev, addr, *addr_type); // FIXME
				    ...
				    loff_t *position)
{
    ...
    /*
       int i;
       if(i_dev_addr == NULL) {
        printk(KERN_INFO "No HCI-Device selected!");
        for(i = 0; i < sizeof(*hci_dev_addr); i++)
            addr.b[i] = 0;
    }
    else {
    printk(KERN_INFO "HCI selected: %pMR", hci_dev_addr);
        for(i = 0; i < sizeof(*hci_dev_addr); i++)
            addr.b[i] = (*hci_dev_addr).b[i];
    }
    printk(KERN_INFO "Using hci %pMR", &addr);*/
		...
		printk(KERN_INFO "Try connecting.");
	...
	/* TODO: Bacc-Arbeit
	 * This let you select the desired hci device
	 */
...
static int __init bt_6lowpan_init(void)
{
    ...
    printk(KERN_INFO "6lowpan module loaded.\n");
    ble_parent = proc_mkdir("ble", NULL);
    ...
    // Proc File System
    lowpan_enable_procfs=proc_create("6lowpan_enable", 0666, ble_parent, &lowpan_enable_fops);
    ...
    // Proc File System
    lowpan_control_procfs=proc_create("6lowpan_control", 0666, ble_parent, &lowpan_control_fops);

    // Now we want to move these files to the proc file system
...
static void __exit bt_6lowpan_exit(void)
{
    ...
    remove_proc_entry("ble_6lowpan_enable", NULL);
    remove_proc_entry("ble_6lowpan_control", NULL);
...
"Christoph Schmidt <christoph.schmidt@student.tugraz.at");
